...
This section documents recent code and behavior changes made to the project so the docs stay up-to-date.

1) Project creation - Client assignment UI
- File: `src/main/java/com/techforge/desktop/ProjectDialog.java`
- Change: The Project form now includes a `JComboBox<Client>` showing available clients in the system. The client loader:
  - Calls `/api/v1/clients` (auth-aware via `ApiClient`).
  - If `/clients` returns empty, it falls back to `/api/v1/users` and filters users whose `role == "CLIENT"`.

  - Robust parsing supports both JSON array responses and Firebase-style map responses. The loader maps `fullName` (preferred) -> `Client.name`, `email` -> `Client.email`, and uses the `id` (or map key) as `Client.id`.
  - The selected client's `id` is saved into the project payload when creating a project: `project.setClientId(selectedClientId)`.
- Notes: The UI renderer shows `Client.name (email)`. The combobox is enabled even on empty results and shows `(No clients found)` placeholder.

2) Finance transaction history (real data)
- File: `src/main/java/com/techforge/erp/service/FinanceService.java`
- Change: `getTransactionHistory()` now reads all `Payroll` records from Firebase, filters to only `paid == true` records, and sorts them newest-first before returning. This ensures frontend `FinancePanel` shows only completed (PAID) transactions instead of hardcoded test values.
- File: `src/main/java/com/techforge/desktop/FinancePanel.java`
- Change: The `loadTransactions()` method now calls the backend `/api/v1/finance/transactions` (via HttpURLConnection), clears previously hardcoded rows, and renders each payroll row as: `[Transaction ID | Employee Name | Type("Payroll") | Amount | Date | Status]`.

3) ApiClient logging and safer error messages
- File: `src/main/java/com/techforge/desktop/ApiClient.java`
- Change: All HTTP methods (GET/POST/PUT/DELETE) now log URL, response code and a truncated response body to the desktop console. Non-2xx responses include the response body in the thrown IOException message to make debugging 403/401 easier.

4) Client login notifications
- File: `src/main/java/com/techforge/desktop/LoginFrame.java`
- Change: After `POST /api/v1/auth/login`, `LoginFrame` now triggers `checkAndNotifyAssignedProjects(clientId, clientName)` when role == `CLIENT`. The method fetches `/projects`, counts active projects where `project.clientId == clientId` and shows a dialog telling the client how many active projects are assigned to them.

5) Small UX/Debug improvements
- The ProjectDialog loader writes debug lines to console to help ops see parsed clients: `[ProjectDialog] Parsed clients count=N` and lists each parsed client's id, name and email.
- `UnifiedLauncher` still launches Spring Boot then the desktop for dev convenience; production deployments should run the backend separately.
